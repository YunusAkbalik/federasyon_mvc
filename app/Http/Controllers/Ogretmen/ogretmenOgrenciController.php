...
class ogretmenOgrenciController extends Controller
{
    public function hesapOlustur(Request $request)
    {
        ...
        if ($sinif != null) {
            ...
            if (!$sinifModel) {
                ...
                if ($sinifModel->kurum_id != get_ogretmen_current_kurum()->id) {
                    $sinif = null;
                    $okul = null;
                } else {
        ...
        return view('ogretmen.hesapOlustur.ogrenci')->with(array(
            'iller' => $iller,
    ...
    public function hesapOlustur_post(Request $request)
    {
        try {
            ...
            $admin = auth()->user();
            $logText = "Öğretmen $admin->ad $admin->soyad ($admin->ozel_id) sisteme öğrenci ekledi ($user->ad $user->soyad ($user->ozel_id))";
            LogModel::create(['kategori_id' => 6, 'logText' => $logText]);

            $kurumLogText = "Öğretmen $admin->ad $admin->soyad ($admin->ozel_id), sisteme öğrenci ekledi ($user->ad $user->soyad ($user->ozel_id))";
            ...
            return redirect()->route('ogretmen_hesapOlustur_ogrenci')->with("success", "Öğrenci kayıt işlemi başarılı");
